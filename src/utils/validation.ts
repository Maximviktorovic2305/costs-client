...
export const validationInputs = (
   ...
   if (isNaN(+priceInputValue)) {
      handleAlertMessage({ alertText: "Введите число!", alertStatus: "error" });
      addDangerBorderByCondition();

      priceInput.current.classList.add("border-accent");
      return false;
   }
   ...
   inputs.forEach((input) => input.classList.remove("border-accent"));
